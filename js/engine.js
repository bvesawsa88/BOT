/* BoTEngine v3 — เอนจินสถานะโต๊ะ Battle of Talingchan (ใช้ร่วม client/server)
   ...
   trigger ที่รองรับ: summoned · activated · static · declareAttack · anyDeclareAttack ·
     enemyDeclareAttack (React) · avatarWouldBeDestroyed (React) · avatarSummoned (React) · lifeRevealedByAttack · destroyed ·
   ...
   op: modifyPower(choose/self/all/equippedAvatar, amountPer, halveFloor) · draw · mill · scout · deckPick ·
     ...
     preventDestroy ·
     sacrifice(cost) · discard(cost) · exileHand(cost) · counterSelf · untapHost · returnSelfToDeck ·
...
(function (root, factory) {
  ...
  const META_RICH_KEYS = [
    ...
    'blockAllLandPlay', 'destroyAfterGlobalEndPhases',
    ...
    'protectAllyNameIncludes', 'attackLimitPerTurn', 'hostCannotAttack', 'instantWinIf', 'stayOnMagic',
    'oncePerTurnCard', 'ignoreReactOncePerTurnLimit'
  ];
  ...
  const INHERIT_META_KEYS = [
    ...
    'attachOnly', 'hostBlockReactUntilCombatEnd', 'suppressVictimDestroyed'
  ];
  ...
  function doMove(st, k, to, pos, fx) {
    ...
    if (/\.(hand|hell|dark|deck)$/.test(to)) {
      ...
      Object.keys(st.inst).forEach(id => {
        ...
        const me = fxCard(m);
        // วูตาตู: โฮสต์ถูกทำลาย → ย้ายสวมใส่ใบอื่น (ถาม) แทนการตกนรก
        if (me && me.reattachOnHostDestroy && (to.endsWith('.hell') || to.endsWith('.dark') || to.endsWith('.deck') || to.endsWith('.hand'))) {
          m.attachedTo = null;
          const own = (from[0] === 'A' || from[0] === 'B') ? from[0] : hostOwner;
          const hosts = (st.zones[own + '.avatar'] || []).filter(h => h !== k);
          if (hosts.length) {
            const p = {
              kind: 'pick', from: 'ids', ids: hosts, src: id, chooser: own,
              dest: 'attachTo', attachMod: id, optional: false, allowAnyZone: true
            };
            st.prompts = st.prompts || [];
            st.prompts.push(p);
            addLog(st, own, `${m.name}: โฮสต์ถูกทำลาย — เลือก Avatar ใหม่เพื่อสวมใส่`);
            return;
          }
        }
        m.attachedTo = null;
  ...
  function wouldDestroyReactOptions(st, owner) {
    return collectReactOptions(st, owner, 'avatarWouldBeDestroyed');
  }
  ...
  function offerWouldDestroyReact(st, fx, k, opts) {
    ...
    const ownOpts = wouldDestroyReactOptions(st, side);
    const oppOpts = wouldDestroyReactOptions(st, opp);
    ...
    st._wouldDestroyPending = { k, opts: Object.assign({}, opts, { ignoreWouldDestroyReact: true }) };
    st.prompts = st.prompts || [];
    st.prompts.unshift({
  ...
  function fireOwnPlayMagic(st, fx, owner, rng) {
  ...
  /* รวบรวม React ในมือ/เซ็ตที่ตอบ trigger ได้ (ยังไม่ใช้ประเภท React ในเทิร์น · ยกเว้นใบข้ามโควต้า) */
  function collectReactOptions(st, owner, triggerOn, filterFn) {
    const reactUsed = isMagicTypeUsed(st, owner, 'React');
    const out = [];
  ...
  function payCostAndRunActivated(st, fx, owner, srcK, costList, actions, rng, onceTag) {
    ...
    if (costList.length) {
      ...
      if (costOp.op === 'discard') {
        ...
        st.prompts.push({ kind: 'chooseDiscard', src: srcK, chooser: owner, filter: filt, actions, effectDiscard: true, discardNeed: need > 1 ? need : undefined, discardGot: 0, excludeIds: [srcK] });
  ...
  function resolvePendingMagic(st, fx, pend, rng) {
    ...
    if (pend.type === 'poorModes') {
      ...
      fireOwnPlayMagic(st, fx, pend.owner, r);
...
function applySelfPowerBuffsFromAb(st, k, ab, logLabel) {
  ...
  function runActions(st, fx, actions, ctx) {
    ...
    (actions || []).forEach(ac => {
      ...
      if (ac.op === 'draw') {
        ...
        else { st.prompts.push(p); prompted = true; addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: เลือกการ์ดในมือทิ้ง ${need} ใบ`); }
        ...
        if (st.inst[ctx.src]) {
          ...
          addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: POWER ${ac.amount > 0 ? '+' : ''}${ac.amount || 0}${until === 'oppNextEnd' ? ' จน End Phase ถัดไปของฝ่ายตรงข้าม' : until === 'nextOwnDraw' ? ' จน Draw Phase ต่อไปของเรา' : until === 'permanent' ? ' ถาวร' : until === 'combat' ? ' จนจบการต่อสู้' : ' จนจบเทิร์น'} → P${effPower(st, ctx.src)}`);
          sweepDestroyPowerZero(st, fx);
        }
        ...
        if (ac.target === 'self') { if (st.inst[ctx.src]) { st.inst[ctx.src].tapped = (ac.op === 'tap'); addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ${ac.op === 'tap' ? 'นอน' : 'ตื่น'}ตัวเอง`); } }
          ...
          // เกษียรสมุทร ฯลฯ: เลือกเทพที่นอนให้ตื่น (ไม่นับตัวเอง)
          const filt = Object.assign({}, ac.filter || {}, { _srcK: ctx.src, excludeSelf: true });
          const p = {
            ...
            optional: ac.optional !== false
          };
          if (promptCandidates(st, p).length) {
            st.prompts.push(p); prompted = true;
            addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: เลือก Avatar ที่นอนให้ตื่น`);
          } else addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ไม่มี Avatar ที่นอนให้ตื่น`);
        }
  ...
  function runAvatarSummonedAbilities(st, fx, k, owner, opts) {
    ...
    if (ab.requireOwnConstructNameIncludes) {
      ...
      if (!opts._skipReact && (opts.paidCost || opts.paidExact) && ab.keyword === 'จุติ' && offerAbilityReact(st, fx, owner, k, { type: 'summoned', k, owner, opts: Object.assign({}, opts, { _onlyAb: ab }) })) {
        st._pendingAbility = { type: 'activate', actions: ab.actions, src: k, owner, costList: normalizeAbilityCost(ab.cost) || (Array.isArray(ab.cost) ? ab.cost : []) };
        return;
      }
      ...
      if (!opts._skipReact && (opts.paidCost || opts.paidExact) && ab.keyword === 'จุติ'
        && offerRichNegateOnJuti(st, fx, owner, k, {
          ...
          actions: ab.actions || [],
          costList: normalizeAbilityCost(ab.cost) || (Array.isArray(ab.cost) ? ab.cost : [])
        })) {
        return;
      }
      // จุติที่มี cost (เช่น ธรณีสูบ) — จ่ายก่อนรันผล
      const costList = normalizeAbilityCost(ab.cost) || (Array.isArray(ab.cost) ? ab.cost : []);
      if (costList.length) payCostAndRunActivated(st, fx, owner, k, costList, ab.actions || [], fx._rng || Math.random);
      else runActions(st, fx, ab.actions, { src: k, owner, toHellAfter: false, rng: fx._rng });
    });
    {
      const e = fxCard(c);
  ...
  function enterChainOrResolve(st, fx, link) {
    ...
    if (srcC && srcC.type === 'Magic') fireOwnPlayMagic(st, fx, link.owner, fx._rng);
  ...
  function resumeJutiAfterRichNegate(st, fx, pend, rng) {
    ...
    if ((pend.type === 'summonedJuti' || pend.type === 'activate') && pend.actions) {
      if (pend.costList && pend.costList.length)
  ...
  function applyAction(st, a) {
    ...
    switch (a.type) {
      ...
      case 'playMagic': {
        ...
        const counterAtk = !!(st.pending && st.pending.target === owner && atkAbs.length);
        ...
        if (atkAbs.length && !counterAtk) {
          const actAb = abilitiesOf(c.code, 'activated', c.name)[0] || abilitiesOf(c.code, 'playMagic', c.name)[0];
          if (!actAb) return deny(`ใช้ "${c.name}" ได้เฉพาะตอนอีกฝ่ายประกาศโจมตี`);
        }
        ...
        if (counterAtk || c.subtype === 'React') {
          if (counterAtk) {
            ...
            doMove(st, a.k, owner + '.hell', null, fx);
            fireOwnPlayMagic(st, fx, owner, rng);
            fx.snd = 'clash'; break;
          }
        ...
        // playMagic = alias ของ activated (เช่น สัญญาเลือด / THE END ใน JSON เก่า)
        const ab = abilitiesOf(c.code, 'activated')[0] || abilitiesOf(c.code, 'playMagic')[0];
        if (!ab) {
          doMove(st, a.k, owner + '.magic', null, fx); c.faceUp = true;
        ...
        if (ab.cost && ab.cost.length) {
          ...
          if (costOp.op === 'discard') {
            ...
            st.prompts.push({
              ...
              gemSumMin: need, gemSumExact: !!costOp.exact, gemGot: 0, actions: ab.actions, effectDiscard: true
            });
          } else if (costOp.op === 'returnHandToDeck') {
      ...
      case 'chooseTarget': {
        ...
        if (p.kind === 'chooseBuff') {
            ...
            if (qd) addLog(st, 'S', `ลงสนามไม่ได้ (${qd}) — ${nameOf(st, a.k)} ขึ้นมือแทน`), doMove(st, a.k, p.chooser + '.hand', null, fx);
              ...
              if (p.multiMax) {
                ...
                if (p.multiGot < p.multiMax && promptCandidates(st, p).length) {
                  st.prompts.unshift(p);
                  addLog(st, p.chooser, `อัญเชิญเพิ่มได้ (${p.multiGot}/${p.multiMax}) หรือข้าม`);
                }
              }
            }
            ...
            if (host) {
              ...
              if (ad) addLog(st, 'S', `"${nameOf(st, a.k)}" ${ad}`);
                ...
                if (p.thenUntap && st.inst[host]) {
                  st.inst[host].tapped = false;
                  addLog(st, p.chooser, `${nameOf(st, host)} ตื่น`);
                }
              }
            } else {
            ...
            if (mod) {
              ...
              if (equipOnto(st, p.attachMod, a.k)) {
                ...
                if (p.thenUntap && st.inst[a.k]) {
                  st.inst[a.k].tapped = false;
                  addLog(st, p.chooser, `${nameOf(st, a.k)} ตื่น`);
                }
          ...
          } else if (p.srcToHell && zoneOf(st, p.src) && !p._skipPickTail) doMove(st, p.src, p.chooser + '.hell', null, fx);
          fx.snd = 'place';
        }
        break;
      }
      ...
      case 'endTurn': {
        ...
        ['avatar', 'magic', 'construct'].forEach(z => (st.zones[st.active + '.' + z] || []).forEach(k => {
          const c = st.inst[k]; if (!c) return;
          // น้ำซุปชาบู: ข้ามการตื่นรอบถัดไปหลังโจมตี
        ...
        if (!st.over) checkInstantWinDraw(st, fx, st.active);
        ...
        // จั่วเสร็จ → เข้า Main ทันที (ไม่ค้างเฟสจั่ว) · เอฟเฟกต์นัด Main รันใน enterPhase
  ...
  return { buildInitialState, applyAction, zoneOf, ownerOf, zLabel, effPower, powerBreakdown, effCost, freeSummonOk, nameMatches, loadEffects, mergeEffects, loadSetReleases, keywordsOf, promptTargetOk, promptCandidates, counterOptions, humanShieldOptions, avatarCap, syncHeimdall, effectOf: (code, nameHint) => resolveEffect(code, nameHint) || EFFECTS[code] || null, hasKw, gemColorOf, gemPaysFor, chooseModeOptionDeny, inOverdose };
});
