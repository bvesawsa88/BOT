...
(function (root, factory) {
  ...
  const INHERIT_META_KEYS = [
    ...
    'attachOnly', 'uniqueMagicNameIncludes', 'hostAttachNameIncludes', 'hostBlockReactUntilCombatEnd', 'suppressVictimDestroyed', 'stackPowerOnReattach',
    ...
    'controlImmuneOwnAvatars', 'linkKeywords', 'setPowerIfAllyNameIncludes', 'setPowerTo'
  ];
  ...
  function protectedFromOppLeave(st, k, chooser) {
    ...
    return (st.zones['land'] || []).some(id => {
      ...
      const landOwn = landControllerOf(st, id, null);
      if (landOwn && landOwn !== side) return false;
  ...
  const hasKw = (st, k, kw) => {
    ...
    for (const id in st.inst) {
      ...
      if (st.inst[id].attachedTo === k && st.inst[id].subtype === 'Modification') {
        ...
        if (kw === 'ลูกฮึด' && /ได้รับ\s*ลูกฮึด|ได้รับความสามารถ\s*ลูกฮึด/.test(txt)) {
          if (nameMatches(st.inst[id], 'ขวาน 7 สี') || nameMatches(st.inst[id], 'ขวานเจ็ดสี')) {
            const host = st.inst[k];
            if (host && host.color === 'เขียว') return true;
            return false;
          }
          return true;
        }
    ...
    if (e && e.grantKeywordIfLandNameIncludes && e.grantKeywordIfLandNameIncludes.keyword === kw) {
      ...
      if (z.endsWith('.avatar')) {
        ...
        const landOk = (st.zones['land'] || []).some(id => st.inst[id] && st.inst[id].faceUp && nameMatches(st.inst[id], g.landNameIncludes || g.nameIncludes || g.name || ''));
        if (landOk) {
          if (g.requireEnemyFaceDownLife || g.condition === 'oppHasFaceDownLife') {
            const opp = other(z[0]);
  ...
  const abilitiesOf = (code, on, nameHint) => {
    const e = resolveEffect(code, nameHint);
    return ((e && e.abilities) || []).filter(ab => {
      if (!ab || !ab.trigger) return false;
      const t = ab.trigger.on;
      if (t === on) return true;
      if (on === 'activated' && (t === 'activatedFromField' || t === 'activatedFromEquipped')) return true;
      return false;
    });
  };
  ...
  function equipOnto(st, modK, hostK) {
    ...
    if (from !== magicZ) {
      const uniq = uniqueMagicNameDeny(st, hostOwn, modK);
  ...
  function controlImmuneBlock(st, targetK, srcK) {
    ...
    // รูปประจำบ้าน: ป้องกัน Avatar เราทั้งหมดจากการเปลี่ยนการควบคุม
    const targetOwner = ownerOf(st, targetK);
    if (targetOwner && targetOwner !== 'S') {
      const protects = (st.zones[targetOwner + '.construct'] || []).some(id => {
        const construct = st.inst[id];
        if (!construct || construct.faceUp === false) return false;
        const e = fxCard(construct);
        return !!(e && e.controlImmuneOwnAvatars);
      });
      if (protects) return `มี "${nameOf(st, targetK)}" ที่ไม่ถูกเปลี่ยนการควบคุมจากความสามารถของ "รูปประจำบ้าน"`;
    }
    return null;
  }
  ...
  function doMove(st, k, to, pos, fx) {
    ...
    if (/\.(hand|hell|dark|deck)$/.test(to)) {
      ...
      if (leaveHost && (leaveHostFx.returnControlOnLeave || (st.inst[k] && st.inst[k].returnControlOnLeave))) {
        if (st.inst[leaveHost] && (zoneOf(st, leaveHost) || '').endsWith('.avatar')) {
          const orig = st.inst[leaveHost].originalOwner || ownerOf(st, leaveHost);
          const curr = ownerOf(st, leaveHost);
          if (orig && orig !== curr) {
            doMove(st, leaveHost, orig + '.avatar', null, fx || {});
            delete st.inst[leaveHost].originalOwner;
            addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, k)}: ออกจากสนาม — คืนการควบคุม ${nameOf(st, leaveHost)} ให้เจ้าของเดิม (${orig})`);
          }
        }
      }
  ...
  function cannotChangeState(st, k) {
    const c = st.inst[k];
    if (!c) return false;
    if (c.cannotChangeStateUntilEOT) return true;
    if (c.cannotChangeStateSourceK) {
      const srcK = c.cannotChangeStateSourceK;
      const srcZ = zoneOf(st, srcK);
      if (srcZ && srcZ.endsWith('.avatar')) return true;
      delete c.cannotChangeStateSourceK;
    }
    return false;
  }
  ...
  function offerWouldDestroyReact(st, fx, k, opts) {
    ...
    if (st._blockDestroyPreventAfterCombat && ownerOf(st, k) === st._blockDestroyPreventFor) return false;
  ...
  function destroyCard(st, fx, k, opts) {
    ...
    if (!wasNullified && !opts.skipDestroyed) {
      abil(st, k, 'destroyed').forEach(ab => {
        if (ab.requireOwnHellMonkeyMin != null) {
          const monkeyCount = (st.zones[side + '.hell'] || []).filter(id => {
            const m = st.inst[id];
            return m && nameMatches(m, 'วานร');
          }).length;
          if (monkeyCount < ab.requireOwnHellMonkeyMin) {
            addLog(st, 'S', `คำสั่งเสีย ${nameOf(st, k)}: "วานร" ในนรกมี ${monkeyCount}/${ab.requireOwnHellMonkeyMin} ใบ — ข้าม`);
            return;
          }
        }
  ...
  function finishSigurdField(st, fx, p, rng) {
    if ((p.multiGot || 0) < 1) return;
    seededShuffle(st.zones[p.chooser + '.deck'], rng);
    addLog(st, p.chooser, 'สับเด็ค');
    const milled = mill(st, fx, p.chooser, 3, rng, 0, p.src);
    addLog(st, p.chooser, 'ธรณีสูบ 3 ใบ');
    const valkyrieCount = (milled || []).filter(id => st.inst[id] && nameMatches(st.inst[id], 'เทพธิดาแห่งวัลฮัลลา')).length;
    const maxCost = 2 + valkyrieCount;
    addLog(st, p.chooser, `ธรณีสูบเจอ "เทพธิดาแห่งวัลฮัลลา" ${valkyrieCount} ใบ → ทำลาย Avatar ฝ่ายตรงข้าม Cost รวมไม่เกิน ${maxCost}`);
    const destroyP = {
      kind: 'pick',
      from: 'enemyAvatars',
      src: p.src,
      chooser: p.chooser,
      dest: 'sigurdDestroyEnemy',
      filter: {},
      costSumMax: maxCost,
      costGot: 0,
      multiMax: 99,
    ...
    if (promptCandidates(st, destroyP).length) {
      st.prompts.unshift(destroyP);
    } else {
      addLog(st, 'S', `ไม่มี Avatar ฝ่ายตรงข้ามที่ Cost รวมไม่เกิน ${maxCost} ให้ทำลาย`);
    }
  }
  ...
  function mill(st, fx, player, count, rng, depth, srcK) {
    ...
    milledIds.forEach(k => {
      ...
      abilitiesOf(st.inst[k].code, 'milled', (st.inst[k] || {}).name).forEach(ab => {
        ...
        (ab.actions || []).forEach(ac => {
          if (ac.op === 'mill' && ac.who === 'both') {
            ...
            if ((zoneOf(st, k) || '') === player + '.hell') {
              doMove(st, k, player + '.dark', null, fx);
              addLog(st, player, `เอฟเฟกต์ ${nameOf(st, k)}: โดนธรณีสูบ — เนรเทศตัวเองไปมิติมืด`);
            }
          } else if (!isMilledBuiltinOp(ac)) rest.push(ac);
        });
        if (rest.length) runActions(st, fx, rest, { src: k, owner: player, rng: rng || (fx && fx._rng) });
      });
      abilitiesOf(st.inst[k].code, 'milledByCardName', (st.inst[k] || {}).name).forEach(ab => {
        const names = (ab.trigger && ab.trigger.names) || [];
        const srcC = st.inst[srcK];
        if (srcC && names.some(n => nameMatches(srcC, n))) {
          const rest = [];
          (ab.actions || []).forEach(ac => {
            if (ac.op === 'banishSelfFromHell') {
              if ((zoneOf(st, k) || '') === player + '.hell') {
                doMove(st, k, player + '.dark', null, fx);
                addLog(st, player, `เอฟเฟกต์ ${nameOf(st, k)}: ถูกธรณีสูบโดย ${srcC.name} — เนรเทศตัวเองไปมิติมืด`);
              }
            } else rest.push(ac);
          });
          if (rest.length) runActions(st, fx, rest, { src: k, owner: player, rng: rng || (fx && fx._rng) });
      ...
      abilitiesOf(st.inst[k].code, 'milledByPurpleGodOrMagic', (st.inst[k] || {}).name).forEach(ab => {
        const srcC = st.inst[srcK];
        if (!srcC) return;
        const syms = cardSymbols(st, srcK) || [];
        const isPurpleGod = srcC.type === 'Avatar' && (srcC.symbol === 'เทพ' || syms.includes('เทพ')) && (srcC.color === 'ม่วง' || srcC.gemColor === 'ม่วง');
        const isMagic = srcC.type === 'Magic';
        if (!isPurpleGod && !isMagic) return;
        const rest = (ab.actions || []).slice();
        if (rest.length) {
          addLog(st, player, `เอฟเฟกต์ ${nameOf(st, k)}: ถูกธรณีสูบด้วย ${isMagic ? 'Magic' : 'Avatar เทพสีม่วง'} (${nameOf(st, srcK)})`);
          runActions(st, fx, rest, { src: k, owner: player, rng: rng || (fx && fx._rng) });
        }
      });
  ...
  /* อาวุธของเพมมุ ฯลฯ — ควบคุมชื่อนี้ใน Magic Zone ได้เพียง 1 ใบ */
  function uniqueMagicNameDeny(st, owner, k) {
    const c = st.inst[k];
    const e = fxCard(c);
    const needle = e && e.uniqueMagicNameIncludes;
    if (!needle || (owner !== 'A' && owner !== 'B')) return null;
    const has = (st.zones[owner + '.magic'] || []).some(id => id !== k && nameMatches(st.inst[id], needle));
    if (has) return `ควบคุม "${needle}" ใน Magic Zone ได้เพียง 1 ใบ`;
    return null;
  }
  ...
  /* สวนกล้วยหนีภาษี ฯลฯ — บล็อกการใช้ Land ทั้งสนาม · มิสทรอม่าจุติล็อกทีละฝ่าย */
  function landPlayBlocked(st, who) {
    if (who && st.blockLandPlayFor && st.blockLandPlayFor[who]) return true;
    return (st.zones['land'] || []).some(id => {
      const c = st.inst[id];
      const e = fxCard(c);
  ...
  function landPlayBlockName(st, who) {
    if (who && st.blockLandPlayFor && st.blockLandPlayFor[who])
      return 'ล็อกการใช้ Land จนจบเทิร์นถัดไปของฝ่ายตรงข้าม';
    for (const id of (st.zones['land'] || [])) {
      const c = st.inst[id];
      const e = fxCard(c);
  ...
  function reactFieldReqOk(st, owner, ab, opts) {
    ...
    if (ab.requireTargetSymbol && opts.targetK) {
      if (!cardSymbols(st, opts.targetK).includes(ab.requireTargetSymbol)) return false;
    }
    if (ab.requireTargetOwn && opts.targetK) {
      if (ownerOf(st, opts.targetK) !== owner) return false;
    }
  ...
  function cannotSelectAttackTarget(st, defId, atkId) {
    ...
    if ((eDef && eDef.cannotBeAttackTargetIfLinked) || (cond && cond.linked)) {
      if (inLinkStatus(st, defId)) return `"${T.name}" อยู่ในสถานะคู่หู ไม่สามารถถูกเลือกเป็นเป้าหมายการโจมตีได้`;
    }
    if (cond) {
  ...
  function normalizeAbilityCost(cost) {
    ...
    const normalizeEntry = c => {
      ...
      if (c.op === 'destroyAlly' || c.op === 'destroyOwnAvatar' || c.op === 'sacrificeAlly') {
        return Object.assign({}, c, { op: 'sacrifice' });
      }
      return c;
    };
    if (Array.isArray(cost)) return cost.map(normalizeEntry);
    if (typeof cost !== 'object') return null;
    if (cost.op === 'destroyAlly' || cost.op === 'destroyOwnAvatar' || cost.op === 'sacrificeAlly') {
      return [{ op: 'sacrifice', filter: cost.filter, count: cost.count || 1 }];
    }
    if (cost.destroyAlly) return [{ op: 'sacrifice', filter: cost.destroyAlly.filter || cost.destroyAlly, count: (cost.destroyAlly && cost.destroyAlly.count) || 1 }];
    if (cost.sacrificeOwnAvatar) return [{ op: 'sacrifice', filter: cost.sacrificeOwnAvatar, count: cost.sacrificeOwnAvatar.count || 1 }];
    if (cost.discardFromHand) return [{ op: 'discard', filter: cost.discardFromHand, count: cost.discardFromHand.count || 1 }];
    if (cost.putOwnAvatarToTopDeck) return [{ op: 'putOwnAvatarToTopDeck', filter: cost.putOwnAvatarToTopDeck }];
    if (cost.sacrificeOtherOwnAvatar) return [{ op: 'sacrificeOtherOwnAvatar', filter: cost.sacrificeOtherOwnAvatar }];
    if (cost.destroyHostAvatar) return [{ op: 'destroyHostAvatar' }];
  ...
  function chooseModeOptionDeny(st, k, owner, opt) {
    ...
    if (opt.requireOwnNameIncludes) {
      ...
      const acNeed = (opt.actions || []).map(x => x.requireOwnNameIncludes || x.ownNameIncludes || x.allyName).find(Boolean);
      if (acNeed) {
  ...
  function effCost(st, k) {
    ...
    if (c.costOverrideUntilEOT != null) return Math.max(0, +c.costOverrideUntilEOT);
  ...
  function powerBreakdown(st, k) {
    ...
    const lockB = (st.buffs || []).find(b => b.k === k && b.lockPrinted);
    if (lockB && (lockB.until === 'combat' || lockB.until === 'endOfTurn' || lockB.until === 'permanent')) {
      let printed = lockB.lockVal != null ? lockB.lockVal : Math.max(0, +c.power || 0);
      ...
      if (zLock.endsWith('.avatar') && c.faceUp) {
        ...
        if (hasHalve) printed = Math.floor(printed / 2);
      }
      return { total: printed, base: printed, lines: [{ amt: printed, label: `ล็อก POWER ตั้งต้นเป็น ${printed}` }], note: 'lock' };
    }
    {
      const e = fxCard(c);
      ...
      sources.forEach(src => {
        ...
        abil(st, src, 'static').forEach(ab => {
          ...
          (ab.actions || []).forEach(ac => {
            ...
            if (ac.op === 'modifyPowerPerMonkeyToken') {
              if (t.select === 'self' && src === k) {
                const monkeyTokens = (st.zones[side + '.avatar'] || []).filter(id => {
                  const m = st.inst[id];
                  return m && m.name === 'วานร' && m.cost === 1 && m.gem === 0;
                }).length;
                const amtTokens = (ac.amountPerToken || 1) * monkeyTokens;
                if (amtTokens) {
                  add(amtTokens, `+${amtTokens} จาก Token วานร ${monkeyTokens} ใบ`);
                  p += amtTokens;
                }
              }
              return;
    ...
    {
      const e = fxCard(c);
      if (e && e.setPowerIfAllyNameIncludes != null && kz.endsWith('.avatar')) {
        const side = kz[0];
        const targetName = typeof e.setPowerIfAllyNameIncludes === 'object' ? (e.setPowerIfAllyNameIncludes.name || e.setPowerIfAllyNameIncludes.nameIncludes) : e.setPowerIfAllyNameIncludes;
        const targetPower = (typeof e.setPowerIfAllyNameIncludes === 'object' && e.setPowerIfAllyNameIncludes.power != null) ? e.setPowerIfAllyNameIncludes.power : e.setPowerTo;
        if (targetName && targetPower != null) {
          const hasAlly = (st.zones[side + '.avatar'] || []).some(id => id !== k && nameMatches(st.inst[id], targetName));
          if (hasAlly) {
            p = targetPower;
            add(targetPower - base, `มี「${targetName}」บนสนาม`);
          }
        }
      }
    }
  ...
  function promptCandidates(st, p) {
    ...
    if (p.kind === 'pick') {
      ...
      else if (p.from === 'ownConstructs') pool = (st.zones[p.chooser + '.construct'] || []).slice();
      ...
      return pool.filter(k => {
        ...
        if ((p.dest === 'bounceHand' || p.dest === 'hell' || p.dest === 'dark' || p.dest === 'destroy' || p.dest === 'sigurdDestroyEnemy' || p.dest === 'takeControl') && protectedFromOppLeave(st, k, p.chooser)) return false;
        ...
        if (p.dest === 'pickAttachHost' && uniqueMagicNameDeny(st, p.chooser, k)) return false;
    ...
    const walk = (actions) => {
      (actions || []).forEach(ac => {
        ...
        if (!freed && actionSummonsToField(ac)) {
          const dummy = summonQuotaDummy(st, ac, srcK);
          const qd = quotaDeny(st, owner + '.avatar', dummy);
          if (qd) { msg = qd + ' — อัญเชิญลงสนามไม่ได้'; return; }
          const ud = uniqueOnFieldDeny(st, owner, dummy);
          if (ud) { msg = ud + ' — อัญเชิญลงสนามไม่ได้'; return; }
        }
        walk(thenActs);
        walk(ac.actions);
      });
  ...
  function activatedTargetDeny(st, owner, ab, srcK, isPlayMagic) {
    ...
    walkEffectActions(ab.actions, ac => {
      if (msg) return true;
      if (ac.op === 'sigurdFieldAbility') {
        const valkyriesInHell = (st.zones[owner + '.hell'] || []).filter(id => st.inst[id] && nameMatches(st.inst[id], 'เทพธิดาแห่งวัลฮัลลา'));
        if (!valkyriesInHell.length) {
          msg = 'ไม่มี "เทพธิดาแห่งวัลฮัลลา" ในนรกเรา';
          return true;
        }
      }
      if (ac.op === 'chooseMode') {
  ...
  function applyMagicPromptOnTarget(st, fx, p, targetK, rng) {
    ...
    if (p.kind === 'chooseDestroy') {
      ...
      const need = p.count || 0;
      const got = (p.got || 0) + 1;
      if (need && got < need) {
        const next = Object.assign({}, p, { got });
        if (promptCandidates(st, next).length) {
          st.prompts.unshift(next);
          addLog(st, p.chooser, `ทำลายเพิ่มได้ (${got}/${need}) หรือข้าม`);
          fx.snd = 'clash';
          return;
        }
      }
  ...
  function collectReactOptions(st, owner, triggerOn, filterFn, reqOpts) {
    ...
    const consider = (m, allowFacedown) => {
      ...
      const isAvatarHandReact = mc.type === 'Avatar' && abilitiesOf(mc.code, triggerOn, mc.name).some(ab => ab.keyword === 'React' && ab.fromHand);
      if (!isAvatarHandReact && (mc.type !== 'Magic' || magicSubtype(mc) !== 'React')) return;
  ...
  function payCostAndRunActivated(st, fx, owner, srcK, costList, actions, rng, onceTag, afterOpts) {
    ...
    if (costList.length) {
      ...
      if (costOp.op === 'discard') {
      ...
      } else if (costOp.op === 'destroyHostAvatar') {
        const c = st.inst[srcK];
        const hostK = c && c.attachedTo;
        if (hostK && st.inst[hostK]) {
          addLog(st, owner, `จ่ายค่า: ทำลาย Avatar ที่สวมใส่อยู่ (${nameOf(st, hostK)})`);
          destroyCard(st, fx, hostK, owner, rng, { src: srcK, cause: 'cost' });
        } else {
          addLog(st, 'S', `จ่ายค่าไม่ได้ — การ์ดไม่ได้สวมใส่ Avatar อยู่`);
          return;
        }
        continueAfterPaidCost(st, fx, contBase, rng);
        ...
        const p = tagCost({
          kind: 'pick', from: 'hell', src: srcK, chooser: owner, filter: filt,
          dest: 'exileHellCost', need, got: 0, actions, optional: false
        });
        if (promptCandidates(st, p).length < need) addLog(st, 'S', `นรกมีไม่ครบ ${need} ใบที่ตรงเงื่อนไขเพื่อเนรเทศ`);
        else {
          st.prompts.push(p);
  ...
  function ownFightFighterK(st, owner) {
    if (!st.pending) return null;
    if (st.pending.by === owner && st.pending.atk) return st.pending.atk;
    if (st.pending.target === owner && st.pending.def) return st.pending.def;
    return null;
  }
  function ownFightReactOptions(st, owner) {
    const fighter = ownFightFighterK(st, owner);
    if (!fighter) return [];
    return (st.zones[owner + '.hand'] || []).filter(k => {
      const c = st.inst[k];
      if (!c) return false;
      const abs = abilitiesOf(c.code, 'ownAvatarFights', c.name);
      if (!abs.length) return false;
      if (c.type === 'Magic') {
        if (reactQuotaBlocks(st, owner, c)) return false;
        if (oncePerTurnCardBlocked(st, k, owner)) return false;
      } else if (c.type !== 'Avatar') return false;
      const ab = abs[0];
      if (ab.requireFighterNameAny) {
        const names = Array.isArray(ab.requireFighterNameAny) ? ab.requireFighterNameAny : [ab.requireFighterNameAny];
        if (!names.some(n => nameMatches(st.inst[fighter], n))) return false;
      }
      if (ab.requireOwnDarkNameMissing) {
        if ((st.zones[owner + '.dark'] || []).some(id => nameMatches(st.inst[id], ab.requireOwnDarkNameMissing)))
          return false;
      }
      return true;
    });
  }
  function offerOwnFightReact(st, fx, side, fighterK) {
    if (!st.pending || !fighterK) return false;
    if (ownerOf(st, fighterK) !== side) return false;
    if ((st.prompts || []).some(p => p.kind === 'react' && p.reactTrigger === 'ownAvatarFights' && p.chooser === side))
      return false;
    const options = ownFightReactOptions(st, side);
    if (!options.length) return false;
    const atkName = nameOf(st, fighterK);
    st.prompts.push({
      kind: 'react', mode: 'runActions', src: null, options, chooser: side, target: fighterK,
      actions: [], fromCounterAtk: true, reactTrigger: 'ownAvatarFights',
      avatarHandAbility: options.some(id => st.inst[id] && st.inst[id].type === 'Avatar'),
      seconds: 10,
      label: `"${atkName}" ฝ่ายเราต่อสู้`
    });
    addLog(st, side, `รอ React (${options.length} ใบ): "${atkName}" ต่อสู้ — เลือกใบ / ไม่ใช้ / รอ 10 วิ`);
    return true;
  }
  function runOwnFightAvatarFromHand(st, fx, p, pick, rng) {
    const m = st.inst[pick];
    if (!m) return;
    const ab = abilitiesOf(m.code, 'ownAvatarFights', m.name)[0];
    const acts = (ab && ab.actions) || p.actions || [];
    const costList = normalizeAbilityCost(ab && ab.cost) || (Array.isArray(ab && ab.cost) ? ab.cost : []);
    addLog(st, p.chooser, `สั่งใช้จากมือ ${m.name} (เมื่อ Avatar ฝ่ายเราต่อสู้)`);
    if (costList.length) payCostAndRunActivated(st, fx, p.chooser, pick, costList, acts, rng);
    else runActions(st, fx, acts, { src: pick, owner: p.chooser, target: p.target, triggerSource: p.target, rng });
    fx.snd = 'place';
  }
  function pemuSpriteCombatLockApplies(st, attackerSide, atkK, defK, isLife, isConstructAtk) {
    if (isLife || isConstructAtk || !defK) return false;
    if (!(st.pemuSpriteCombatLock && st.pemuSpriteCombatLock[attackerSide])) return false;
    const A = st.inst[atkK];
    if (!A || !(nameMatches(A, 'เพมมุ') || nameMatches(A, 'สไปรท์'))) return false;
    const D = st.inst[defK];
    if (!D || D.type !== 'Avatar') return false;
    return ownerOf(st, defK) === other(attackerSide);
  }
  function armDestroyPreventFromPending(st, pnd) {
    if (pnd && pnd.blockDestroyPreventAfter) {
      st._blockDestroyPreventAfterCombat = true;
      st._blockDestroyPreventFor = pnd.target;
    }
  }
  function clearDestroyPreventLock(st) {
    delete st._blockDestroyPreventAfterCombat;
    delete st._blockDestroyPreventFor;
  }
  ...
  function resolvePendingMagic(st, fx, pend, rng) {
    ...
    if (pend.type === 'poorModes') {
      ...
      if (c && zoneOf(st, pend.src)) {
        if (landPlayBlocked(st, pend.owner)) {
...
function applySelfPowerBuffsFromAb(st, k, ab, logLabel) {
  ...
  function declareEffects(st, fx, atkId, defId, rng) {
    ...
    const runDeclare = (srcK, label) => {
      abil(st, srcK, 'declareAttack').forEach(ab => {
        ...
        if (cond === 'hostColor==แดง') {
          const host = st.inst[srcK] && st.inst[srcK].attachedTo;
          if (!host || st.inst[host].color !== 'แดง') return;
        }
        if (cond === 'hostColor==ฟ้า') {
          const host = st.inst[srcK] && st.inst[srcK].attachedTo;
          if (!host || st.inst[host].color !== 'ฟ้า') return;
        }
        if (cond === 'hostColor==ม่วง') {
          const host = st.inst[srcK] && st.inst[srcK].attachedTo;
          if (!host || st.inst[host].color !== 'ม่วง') return;
        }
  ...
  function runActions(st, fx, actions, ctx) {
    ...
    (actions || []).forEach(ac => {
      ...
      if (ac.op === 'draw') {
        ...
        players.forEach(p => {
          let n = count;
          if (ac.untilHand != null)
            n = Math.max(0, ac.untilHand - ((st.zones[p + '.hand'] || []).length));
          const got = takeFromDeckToHand(st, p, n, fx);
          if (got.length) addLog(st, p, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: จั่ว ${got.length} ใบ`);
          else if (ac.untilHand != null)
            addLog(st, p, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: มือครบ ${ac.untilHand} แล้ว`);
        });
      ...
      } else if (ac.op === 'destroyEnemyCostScaledByHellOrRevealLife') {
        const opp = other(ctx.owner);
        const hellCount = (st.zones[ctx.owner + '.hell'] || []).length;
        const maxCost = 3 + Math.floor(hellCount / 10);
        const candidates = (st.zones[opp + '.avatar'] || []).filter(id => {
          const c = st.inst[id];
          return c && effCost(st, id) <= maxCost && !protectedFromOppLeave(st, id, ctx.owner);
        });
        if (!candidates.length) {
          addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ไม่มี Avatar Cost ≤ ${maxCost} ฝ่ายตรงข้ามให้ทำลาย → หงาย LIFE Card ใบบนสุดฝ่ายตรงข้าม`);
          const life = st.zones[opp + '.life'] || [];
          const top = life.find(id => st.inst[id] && !st.inst[id].faceUp) || null;
          if (top) {
            st.inst[top].faceUp = true;
          ...
          st.prompts.push({
            ...
            options: [
              {
                label: `ทำลาย Avatar ฝ่ายตรงข้าม (Cost ≤ ${maxCost})`,
                actions: [{
                  op: 'chooseDestroy',
                  filter: { type: 'Avatar', costMax: maxCost },
                  zones: ['avatar'],
                  side: 'enemy',
                  dest: 'destroy'
                }]
              ...
              {
                label: `หงาย LIFE Card ใบบนสุดของฝ่ายตรงข้าม 1 ใบ`,
      ...
      } else if (ac.op === 'millEnemy') {
        const opp = other(ctx.owner);
        const n = ac.count || 1;
        addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ฝ่ายตรงข้ามธรณีสูบ ${n} ใบ`);
        mill(st, fx, opp, n, ctx.rng, 0, ctx.src);
      ...
      } else if (ac.op === 'summonSelfFromDark' || ac.op === 'summonSelfFromDarkDimension') {
        const c = st.inst[ctx.src];
        const z = zoneOf(st, ctx.src) || '';
      ...
      } else if (ac.op === 'destroyEnemyLowestCost') {
        const opp = other(ctx.owner);
        const enemyAvatars = (st.zones[opp + '.avatar'] || []).map(id => st.inst[id]).filter(c => c && c.type === 'Avatar');
        if (!enemyAvatars.length) {
          addLog(st, 'S', `ไม่มี Avatar ฝ่ายตรงข้ามบนสนาม`);
        } else {
          const minCost = Math.min(...enemyAvatars.map(c => effCost(st, c.id)));
          const targets = enemyAvatars.filter(c => effCost(st, c.id) === minCost).map(c => c.id);
          addLog(st, ctx.owner, `พาลี: ทำลาย Avatar ฝ่ายตรงข้ามที่มี Cost ต่ำที่สุด (${minCost}) จำนวน ${targets.length} ใบ`);
          targets.forEach(id => destroyCard(st, fx, id));
        }
      } else if (ac.op === 'setChosenEnemyCostsToLowest') {
        const opp = other(ctx.owner);
        const cands = (st.zones[opp + '.avatar'] || []).filter(id => st.inst[id] && st.inst[id].type === 'Avatar');
        if (cands.length < 2) {
          addLog(st, 'S', `สุครีพ: Avatar ฝ่ายตรงข้ามมีน้อยกว่า 2 ใบ — ปรับ Cost ไม่ได้`);
        } else {
          st.prompts.push({
            kind: 'pick', from: 'ids', ids: cands, src: ctx.src, chooser: ctx.owner,
            dest: 'setCostsToLowest', multiExact: 2, optional: false
          });
          prompted = true;
          addLog(st, ctx.owner, `สุครีพ: เลือก Avatar ฝ่ายตรงข้าม 2 ใบปรับ Cost เท่าใบต่ำสุด`);
        }
      } else if (ac.op === 'scoutAndPickOrSummonKingka') {
        let count = ac.count || 7;
        let ids = (st.zones[ctx.owner + '.deck'] || []).slice(-Math.min(count, (st.zones[ctx.owner + '.deck'] || []).length)).reverse();
        if (!ids.length) {
          addLog(st, 'S', `เลือกตั้งเจ้าป่า: เด็คว่าง — สอดแนมไม่ได้`);
        } else {
          st.prompts.push({
            kind: 'pick', from: 'ids', ids, src: ctx.src, chooser: ctx.owner, filter: ac.filter || {},
            dest: 'scoutPickOrSummonKingka', restTo: 'bottom', optional: true, revealAllScout: true
          });
          lockScoutIds(st, ids);
          fx.scoutView = { p: ctx.owner, n: ids.length };
          prompted = true;
        ...
        let from = ac.from === 'own' ? 'ownAvatars' : ac.from === 'enemy' ? 'enemyAvatars' : ac.from === 'any' ? 'allAvatars' : ac.from === 'ownMagic' ? 'ownMagic' : ac.from === 'ownConstructs' ? 'ownConstructs' : 'ownAvatars';
        const p = {
      ...
      } else if (ac.op === 'setBasePower') {
        const targetSel = (ac.target && ac.target.select) || 'attacker';
        const tgtK = targetSel === 'attacker' ? ctx.attacker : (targetSel === 'foe' ? ctx.foe : (ac.target && ac.target.id));
        if (tgtK && st.inst[tgtK]) {
          const val = ac.value != null ? ac.value : 0;
          const until = ac.duration === 'endOfCombat' || ac.duration === 'combat' ? 'combat' : 'endOfTurn';
          st.buffs = st.buffs || [];
          st.buffs.push({ k: tgtK, lockPrinted: true, lockVal: val, amt: 0, until, from: ctx.src });
          addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ปรับ POWER ตั้งต้นของ ${nameOf(st, tgtK)} เป็น ${val}`);
        }
        ...
        const p = {
          ...
          count: ac.count || ac.max || null, got: 0
        };
        if (promptCandidates(st, p).length) { st.prompts.push(p); prompted = true; }
        else addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ไม่มีเป้าหมายให้ทำลาย — ข้าม`);
      } else if (ac.op === 'armPemuSpriteCombatLock') {
        st.pemuSpriteCombatLock = st.pemuSpriteCombatLock || {};
        st.pemuSpriteCombatLock[ctx.owner] = true;
        addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: เทิร์นนี้เมื่อเพมมุ/สไปรท์โจมตี Avatar ศัตรู ฝ่ายตรงข้ามใช้ React/ความสามารถไม่ได้จนกว่าจะจบการต่อสู้`);
      } else if (ac.op === 'lockOppLandPlay') {
        const opp = other(ctx.owner);
        st.blockLandPlayFor = st.blockLandPlayFor || {};
        st.blockLandPlayFor[opp] = true;
        addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ฝ่ายตรงข้ามใช้ Land ไม่ได้จนจบเทิร์นถัดไปของฝ่ายตรงข้าม`);
      } else if (ac.op === 'destroyHighestPower') {
        const ot = other(ctx.owner);
        const ids = (st.zones[ot + '.avatar'] || []).filter(id => {
          if (!st.inst[id] || st.inst[id].faceUp === false) return false;
          if (protectedFromOppLeave(st, id, ctx.owner)) return false;
          return true;
        });
      ...
      } else if (ac.op === 'scout' || ac.op === 'scoutAndSummon' || ac.op === 'scoutAndSummonAll') {
        ...
        if (ac.thenDestroyEnemyCostSumLte) {
          ...
          const pickFilter = (ac.dest === 'nongSam' && ac.buffFromNameIncludes)
            ...
            : ac.filter;
          const isSummonScout = ac.op === 'scoutAndSummon' || ac.op === 'scoutAndSummonAll';
          const rawDest = ac.dest === 'avatarZone' ? 'avatar' : ac.dest;
          const dest = ac.dest === 'nongSam' ? 'hand' : (rawDest || (isSummonScout ? 'avatar' : 'hand'));
          /* restTo: deck = คืนเข้าเด็คแล้วสับ (เทียบเท่า bottom + shuffle) */
          let restTo = ac.restTo === 'choose' ? 'bottom' : (ac.restTo === 'deckBottom' ? 'bottom' : (ac.restTo || (ac.restToBottom ? 'bottom' : 'bottom')));
      ...
      } else if (ac.op === 'hellPick' || ac.op === 'hellPickIfAllyOnField') {
        if (ac.op === 'hellPickIfAllyOnField' && ac.allyName) {
          const hasAlly = (st.zones[ctx.owner + '.avatar'] || []).some(id => nameMatches(st.inst[id], ac.allyName));
          if (!hasAlly) {
            addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ต้องมี "${ac.allyName}" บนสนาม`);
            return;
          }
        }
        let filter = Object.assign({}, ac.filter || {});
      ...
      } else if (ac.op === 'sigurdFieldAbility') {
        const hellValks = (st.zones[ctx.owner + '.hell'] || []).filter(id => st.inst[id] && nameMatches(st.inst[id], 'เทพธิดาแห่งวัลฮัลลา'));
        if (!hellValks.length) {
          addLog(st, 'S', 'ไม่มี "เทพธิดาแห่งวัลฮัลลา" ในนรกเรา');
          return;
        }
        const p = {
          kind: 'pick', from: 'hell', src: ctx.src, chooser: ctx.owner,
          filter: { nameIncludes: ['เทพธิดาแห่งวัลฮัลลา'] },
          dest: 'sigurdReturnHell', optional: false, multiMax: 3, multiMin: 1, multiGot: 0,
          returnedIds: []
        };
        st.prompts.push(p); prompted = true;
        addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: เลือก "เทพธิดาแห่งวัลฮัลลา" จากนรกกลับเข้าเด็ค 1-3 ใบ`);
      } else if (ac.op === 'summon' || ac.op === 'summonFromHell') {
        const fromZone = (ac.from === 'hell' || ac.op === 'summonFromHell') ? 'hell' : 'deckAll';
        const p = { kind: 'pick', from: fromZone, src: ctx.src, chooser: ctx.owner, filter: ac.filter, dest: 'avatar', shuffleAfter: fromZone !== 'hell', optional: false };
        ...
        const chooser = ac.who === 'opp' ? other(ctx.owner) : ctx.owner;
        const receiver = ctx.owner;
        const p = {
          kind: 'pick', from: ac.from === 'own' ? 'ownAvatars' : 'enemyAvatars', src: ctx.src, chooser, receiver, filter: ac.filter,
          ...
          changeSrcSubtype: ac.changeSrcSubtype || null,
          returnControlOnLeave: !!ac.returnControlOnLeave
        };
        if (promptCandidates(st, p).length) { st.prompts.push(p); prompted = true; }
        else addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ไม่มี Avatar ให้ยึด`);
      } else if (ac.op === 'exile') {
        ...
        if (ac.target === 'self') {
          ...
          const p = {
            ...
            then: ac.then || null,
            untilSourceLeavesZone: !!ac.untilSourceLeavesZone
          };
          const cands = promptCandidates(st, p);
          if (cands.length < need) {
      ...
      } else if (ac.op === 'toggleTap') {
        const filt = Object.assign({}, ac.filter || {}, { _srcK: ctx.src });
        const p = {
          kind: 'pick', from: ac.from === 'enemy' ? 'enemyAvatars' : 'ownAvatars',
          src: ctx.src, chooser: ctx.owner, filter: filt, dest: 'toggleTap',
          optional: ac.optional != null ? !!ac.optional : false
        };
        const cands = promptCandidates(st, p);
        if (!cands.length) addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: ไม่มีเป้าให้เปลี่ยนสภาพ`);
        else if (cands.length === 1) {
          const t = cands[0];
          if (cannotChangeState(st, t)) addLog(st, 'S', `${nameOf(st, t)} ไม่สามารถเปลี่ยนสภาพได้จนจบเทิร์น`);
          else {
            st.inst[t].tapped = !st.inst[t].tapped;
            addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: เปลี่ยนสภาพ ${nameOf(st, t)} → ${st.inst[t].tapped ? 'นอน' : 'ตื่น'}`);
          }
        } else {
          st.prompts.push(p); prompted = true;
          addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: เลือก Avatar เปลี่ยนสภาพ`);
        }
        ...
        const needle = (ac.filter && ac.filter.nameIncludes) || 'ไวรัส';
        const min = ac.min || 5;
        const count = (st.zones[ctx.owner + '.dark'] || []).filter(id => {
          const dc = st.inst[id]; return dc && nameMatches(dc, needle);
        }).length;
        if (count >= min) {
          const p = {
            kind: 'pick', from: 'dark', src: ctx.src, chooser: ctx.owner,
            filter: ac.filter || { nameIncludes: ['ไวรัส'] },
            dest: 'avatar', paidCost: false, optional: false
          };
          if (promptCandidates(st, p).length) {
            st.prompts.push(p);
            prompted = true;
            addLog(st, ctx.owner, `เอฟเฟกต์ ${nameOf(st, ctx.src)}: มิติมืดมี "${needle}" ≥ ${min} ใบ — เลือกอัญเชิญจากมิติมืด`);
          } else addLog(st, 'S', `ไม่มีการ์ดในมิติมืดที่ตรงเงื่อนไขให้อัญเชิญ`);
        } else {
          addLog(st, 'S', `เอฟเฟกต์ ${nameOf(st, ctx.src)}: มิติมืดมี "${needle}" ${count} ใบ (ไม่ถึง ${min} ใบ)`);
        }
      }
    });
  ...
  function unmarkMagicTypeUsed(st, player, mtype, cardName) {
    if (st.magicUsed && st.magicUsed[player]) {
      delete st.magicUsed[player][mtype];
    }
  }
  ...
  function runAvatarSummonedAbilities(st, fx, k, owner, opts) {
    ...
    abil(st, k, 'summoned').forEach(ab => {
      ...
      if (ab.requireOwnHellMonkeyMin != null) {
        const monkeyCount = (st.zones[owner + '.hell'] || []).filter(id => {
          const m = st.inst[id];
          return m && nameMatches(m, 'วานร');
        }).length;
        if (monkeyCount < ab.requireOwnHellMonkeyMin) {
          addLog(st, 'S', `จุติ ${nameOf(st, k)}: "วานร" ในนรกมี ${monkeyCount}/${ab.requireOwnHellMonkeyMin} ใบ — ข้าม`);
          return;
        }
      }
      if (ab.requireOwnNameIncludes) {
  ...
  function offerWhenAttacking(st, atkId) {
    ...
    if (!costList.length) return false;
    const costOp = costList[0];
  ...
  function resolveCombat(st, fx, atkId, defId, lifeId) {
    ...
    if (lifeId) {
      ...
      abilitiesOf(L.code, 'lifeRevealedByAttack', L.name).forEach(ab => {
        if (ab.requireOwnHellMonkeyMin != null) {
          const monkeyCount = (st.zones[lifeOwner + '.hell'] || []).filter(id => st.inst[id] && nameMatches(st.inst[id], 'วานร')).length;
          if (monkeyCount < ab.requireOwnHellMonkeyMin) {
            addLog(st, 'S', `เอฟเฟกต์ LIFE "${L.name}": "วานร" ในนรกมี ${monkeyCount}/${ab.requireOwnHellMonkeyMin} ใบ — ข้าม`);
            return;
          }
        }
        (ab.actions || []).forEach(ac => {
    ...
    const fireBattleDestroy = (winnerId, victimId, extra) => {
      ...
      const runBattle = (srcK) => {
        abil(st, srcK, 'battleDestroy').forEach(ab => {
          ...
          if (ab.requireLinked && !inLinkStatus(st, srcK)) return;
          if (ab.requireLandNameIncludes) {
            const ok = (st.zones['land'] || []).some(id => st.inst[id] && st.inst[id].faceUp && nameMatches(st.inst[id], ab.requireLandNameIncludes));
            if (!ok) return;
          }
  ...
  function buildInitialState(cards, rng, decks, opts) {
    ...
    const mk = (c, faceUp, owner) => {
      const k = 'i' + (++n);
      inst[k] = { id: k, code: c.code, name: c.name, type: c.type, subtype: magicSubtype(c) || c.subtype || '', symbol: c.symbol || '', color: c.color || '', gemColor: c.gemColor || '', cost: c.cost, gem: c.gem, power: c.power, ex: c.ex || '', effect: c.effect || '—', img: c.imageUrl || (c.image ? `https://cdn.bangbon.app/cards/${c.image}` : (c.code ? `https://cdn.bangbon.app/cards/${c.code}.png` : '')), faceUp: faceUp !== false, tapped: false, counters: 0, attachedTo: null };
  ...
  function applyAction(st, a) {
    ...
    let rollbackMagicType = null;
    const deckSide = v => (isPlayer ? by : (v === 'B' ? 'B' : 'A'));
    const deny = m => {
      if (rollbackMagicType) {
        unmarkMagicTypeUsed(st, rollbackMagicType.player, rollbackMagicType.mtype, rollbackMagicType.cardName);
        rollbackMagicType = null;
      }
      fx.deny = m;
      return fx;
    };
    ...
    switch (a.type) {
      ...
      case 'move': {
        ...
        if (to === 'land') {
          ...
          const landWho = (from[0] === 'A' || from[0] === 'B') ? from[0] : null;
          if (landPlayBlocked(st, landWho) && !(st.zones['land'] || []).includes(a.k))
            return deny(`ใช้ Land ไม่ได้ — "${landPlayBlockName(st, landWho)}" บล็อกการใช้ Land`);
      ...
      case 'destroyCard': {
        ...
        destroyCard(st, fx, a.k, { ignoreProtect: !!a.ignoreProtect });
        break;
      }
      ...
      case 'summon': {
        ...
        const paidCost = a.paidCost != null ? !!a.paidCost : (!!a.free ? false : (cost === 0 || usable >= cost));
      ...
      case 'playMagic': {
        ...
        if (enforceType) {
          ...
          rollbackMagicType = { player: owner, mtype, cardName: c && c.name };
        ...
        if (counterAtk || magicSubtype(c) === 'React') {
          if (counterAtk) {
            ...
            if (costList && costList[0] && (costList[0].op === 'banishFromHell' || costList[0].op === 'exileHell')) {
              const need = costList[0].count || 1;
              const filt = costList[0].filter || {};
              const p = {
                kind: 'pick', from: 'hell', src: a.k, chooser: owner,
                filter: filt, dest: 'exileHellCost', need, got: 0,
                actions: acts, optional: false, keepSrc: true,
                counterAtkCtx: { atk: atkId, def: defId }
              };
              if (promptCandidates(st, p).length < need) {
                addLog(st, 'S', `ใช้ "${c.name}" ไม่ได้ — นรกมีไม่ครบ ${need} ใบที่ตรงเงื่อนไขเพื่อเนรเทศ`);
                doMove(st, a.k, owner + '.hell', null, fx);
                fx.snd = 'clash'; break;
              }
              st.prompts.push(p);
              st.reactCleanup = { src: a.k, owner: owner, atk: atkId };
              addLog(st, owner, `การ์ดสวน "${c.name}": เนรเทศจากนรก ${need} ใบเพื่อจ่ายค่า`);
              fx.snd = 'place'; break;
            }
        ...
        if (c.subtype === 'Land') {
          if (landPlayBlocked(st, owner))
            return deny(`ใช้ Land ไม่ได้ — "${landPlayBlockName(st, owner)}" บล็อกการใช้ Land`);
          doMove(st, a.k, owner + '.magic', null, fx); c.faceUp = true;
        ...
        const ab = (c.subtype === 'Modification')
          ...
          : (abilitiesOf(c.code, 'activated')[0] || abilitiesOf(c.code, 'activatedFromHand')[0] || abilitiesOf(c.code, 'playMagic')[0] || abilitiesOf(c.code, 'chooseMode')[0]);
        if (!ab) {
          doMove(st, a.k, owner + '.magic', null, fx); c.faceUp = true;
        ...
        if (costList.length) {
          ...
          if (costOp.op === 'discard') {
            ...
            const filt = costOp.filter || {};
            const p = { kind: 'pick', from: 'ownAvatars', src: a.k, chooser: owner, filter: filt, dest: 'sacrifice', actions: ab.actions, optional: false, afterCostKind: 'magic' };
            if (!promptCandidates(st, p).length) {
              const reqName = filt.nameIncludes ? (Array.isArray(filt.nameIncludes) ? filt.nameIncludes.join('/') : filt.nameIncludes) : '';
              return deny(reqName ? `ใช้ "${c.name}" ไม่ได้ — ต้องมี Avatar "${reqName}" บนสนามฝ่ายเราเพื่อทำลาย/เซ่นไหว้` : `ใช้ "${c.name}" ไม่ได้ — ไม่มีการ์ดบนสนามให้เซ่นไหว้`);
            }
            doMove(st, a.k, owner + '.magic', null, fx);
            addLog(st, owner, `ใช้เวท ${c.name} — เลือกการ์ดเซ่นไหว้/ทำลาย`);
            st.prompts.push(p);
          } else if (costOp.op === 'sacNamedUnique') {
            if (!sacNamedUniqueOk(st, owner, costOp))
          ...
          } else if (costOp.op === 'banishFromHell' || costOp.op === 'exileHell') {
            const need = costOp.count || 1;
            const filt = costOp.filter || {};
            const p = {
              kind: 'pick', from: 'hell', src: a.k, chooser: owner, filter: filt,
              dest: 'exileHellCost', need, got: 0, actions: ab.actions, optional: false, afterCostKind: 'magic'
            };
            if (promptCandidates(st, p).length < need) return deny(`ใช้ "${c.name}" ไม่ได้ — นรกมีไม่ครบ ${need} ใบที่ตรงเงื่อนไขเพื่อเนรเทศ`);
            doMove(st, a.k, owner + '.magic', null, fx);
            addLog(st, owner, `ใช้เวท ${c.name} — เนรเทศจากนรก ${need} ใบเพื่อจ่ายค่า`);
            st.prompts.push(p);
          } else if (costOp.op === 'mill' || costOp.op === 'millSelf') {
            const n = costOp.count || 1;
            const who = costOp.who === 'opp' ? other(owner) : owner;
            if ((st.zones[who + '.deck'] || []).length < n) return deny(`ใช้ "${c.name}" ไม่ได้ — เด็คไม่พอธรณีสูบ ${n} ใบ`);
            doMove(st, a.k, owner + '.magic', null, fx);
            addLog(st, owner, `ใช้เวท ${c.name} — จ่ายค่า: ธรณีสูบ ${n} ใบ`);
            mill(st, fx, who, n, rng, 0, a.k);
            const magicActions = (ab && ab.actions) || (ab && ab.options ? [{ op: 'chooseMode', options: ab.options }] : []);
            if (offerMagicNegateReact(st, fx, owner, a.k)) {
              st._pendingMagic = { type: 'activated', src: a.k, owner, actions: magicActions, toHellAfter: magicHellAfterPlay(c) };
              break;
            }
            enterChainOrResolve(st, fx, { src: a.k, owner, actions: magicActions });
            fireEnemyActivate(st, fx, owner, rng);
            fx.snd = 'place'; break;
          } else {
            return deny(`ใช้ "${c.name}" ไม่ได้ — ระบบยังไม่รองรับค่าใช้จ่าย (${costOp.op})`);
          }
      ...
      case 'attach': {
        ...
        {
          const uniq = uniqueMagicNameDeny(st, from[0], a.k);
          if (uniq) return deny(uniq);
        }
        {
          const he = fxCard(host);
      ...
      case 'chooseTarget': {
        ...
        if (p.kind === 'react') {
          ...
          if (p.reactTrigger === 'enemyDeclareAttack' || p.reactTrigger === 'oppBattlePhaseStart' || p.reactTrigger === 'ownAvatarFights' || p.reactTrigger === 'ownAvatarLeftField') {
            ...
            st.prompts.shift();
            if (p.reactTrigger === 'ownAvatarFights' && st.inst[a.k] && st.inst[a.k].type === 'Avatar') {
              runOwnFightAvatarFromHand(st, fx, p, a.k, rng);
              break;
            }
        ...
        if (p.kind === 'chooseBuff') {
          ...
          if (p.dest === 'sacrifice') {
            ...
            } else if (landPlayBlocked(st, p.chooser)) {
              addLog(st, 'S', `เล่น Land ไม่ได้ — "${landPlayBlockName(st, p.chooser)}" บล็อกการใช้ Land`);
            } else {
              clearLandZoneFor(st, fx, a.k);
            ...
            if (p.got < (p.need || 1) && promptCandidates(st, p).length) {
              ...
              if (p.afterCostKind) continueAfterPaidCost(st, fx, { src: p.src, owner: p.chooser, actions: p.actions || [], kind: p.afterCostKind, keepSrc: !!p.keepSrc, counterAtkCtx: p.counterAtkCtx }, rng);
              else {
                const actCtx = { src: p.src, owner: p.chooser, rng };
                if (p.counterAtkCtx) { actCtx.attacker = p.counterAtkCtx.atk; actCtx.target = p.counterAtkCtx.def; }
                runActions(st, fx, p.actions || [], actCtx);
              }
            }
            ...
            if (block) { addLog(st, 'S', block); }
              ...
              const targetSide = p.receiver || p.chooser;
              const qd = quotaDeny(st, targetSide + '.avatar', st.inst[a.k]);
              if (qd) addLog(st, 'S', `ยึดไม่ได้ (${qd})`);
                ...
                doMove(st, a.k, targetSide + '.avatar', null, fx);
                if (p.keepTapped !== false) st.inst[a.k].tapped = true;
                addLog(st, p.chooser, `⛓️ เอฟเฟกต์ ${nameOf(st, p.src)}: ยึดการควบคุม ${nameOf(st, a.k)} มาฝั่ง ${targetSide}${p.keepTapped !== false ? ' (นอน)' : ''}`);
                if (p.changeSrcSubtype && st.inst[p.src]) st.inst[p.src].subtype = p.changeSrcSubtype;
                if (p.thenAttachSrc && st.inst[p.src] && equipOnto(st, p.src, a.k))
                  addLog(st, p.chooser, `เอฟเฟกต์ ${nameOf(st, p.src)}: สวมใส่ตัวเองให้ ${nameOf(st, a.k)}`);
                if (p.returnControlOnLeave && st.inst[p.src]) st.inst[p.src].returnControlOnLeave = true;
            ...
            if (p.then && p.then.length) runActions(st, fx, p.then, { src: p.src, owner: p.chooser, rng });
          } else if (p.dest === 'setCostsToLowest') {
            p.pickedIds = p.pickedIds || [];
            if (!p.pickedIds.includes(a.k)) p.pickedIds.push(a.k);
            if (p.pickedIds.length < (p.multiExact || 2)) {
              st.prompts.unshift(p);
              addLog(st, p.chooser, `สุครีพ: เลือก Avatar ฝ่ายตรงข้ามใบที่ 2 (${p.pickedIds.length}/2)`);
            } else {
              const minCost = Math.min(...p.pickedIds.map(id => effCost(st, id)));
              p.pickedIds.forEach(id => {
                if (st.inst[id]) {
                  st.inst[id].costOverrideUntilEOT = minCost;
                  addLog(st, p.chooser, `สุครีพ: ${nameOf(st, id)} ปรับ Cost เป็น ${minCost} จนจบเทิร์น`);
                }
              });
            }
          } else if (p.dest === 'scoutPickOrSummonKingka') {
            const c = st.inst[a.k];
            const isKingka = c && nameMatches(c, 'คิงคา');
            const qd = quotaDeny(st, p.chooser + '.avatar', c);
            if (isKingka && !qd) {
              doMove(st, a.k, p.chooser + '.avatar', null, fx);
              addLog(st, p.chooser, `เลือกตั้งเจ้าป่า: เลือก ${nameOf(st, a.k)} → อัญเชิญ จุติ ลง Avatar Zone`);
              triggerSummon(st, fx, a.k, p.chooser, { paidCost: true });
              fx.snd = 'place';
            } else {
              doMove(st, a.k, p.chooser + '.hand', null, fx);
              addLog(st, p.chooser, `เลือกตั้งเจ้าป่า: เลือก ${nameOf(st, a.k)} ขึ้นมือ`);
            }
          } else if (p.dest === 'pickTuyenConstruct') {
            const constructK = a.k;
            const constructPower = effPower(st, constructK);
            addLog(st, p.chooser, `เลือก Construct: ${nameOf(st, constructK)} (POWER ${constructPower})`);
            const opp = other(p.chooser);
            const candidates = (st.zones[opp + '.avatar'] || []).filter(id => {
              const c = st.inst[id];
              return c && effPower(st, id) <= constructPower;
            });
            if (candidates.length === 0) {
              addLog(st, 'S', `ไม่มี Avatar ฝ่ายตรงข้ามที่มี POWER ≤ ${constructPower}`);
            } else {
              st.prompts.unshift({
                kind: 'pick',
                from: 'ids',
                ...
                dest: 'destroy',
                optional: false,
                label: `ทำลาย Avatar ที่มี POWER ≤ ${constructPower}`
              });
            }
            ...
            if (p.dest === 'untap') {
              ...
              if (cannotChangeState(st, a.k))
                ...
                if (p.untilSourceLeavesZone) st.inst[a.k].cannotChangeStateSourceK = p.src;
          ...
          } else if (p.dest === 'toggleTap') {
            if (cannotChangeState(st, a.k))
              addLog(st, 'S', `${nameOf(st, a.k)} ไม่สามารถเปลี่ยนสภาพได้จนจบเทิร์น`);
            else {
              st.inst[a.k].tapped = !st.inst[a.k].tapped;
              addLog(st, p.chooser, `เอฟเฟกต์ ${nameOf(st, p.src)}: เปลี่ยนสภาพ ${nameOf(st, a.k)} → ${st.inst[a.k].tapped ? 'นอน' : 'ตื่น'}`);
            }
          ...
          } else if (p.dest === 'sigurdReturnHell') {
            doMove(st, a.k, p.chooser + '.deck', null, fx);
            p.returnedIds = p.returnedIds || [];
            p.returnedIds.push(a.k);
            p.multiGot = (p.multiGot || 0) + 1;
            addLog(st, p.chooser, `เอฟเฟกต์ ${nameOf(st, p.src)}: นำ ${nameOf(st, a.k)} จากนรกกลับเด็ค (${p.multiGot}/3)`);
            if (p.multiGot < 3 && promptCandidates(st, p).length) {
              p.optional = true;
              st.prompts.unshift(p);
              addLog(st, p.chooser, `เลือกเพิ่มได้ (${p.multiGot}/3) หรือข้าม`);
            } else {
              finishSigurdField(st, fx, p, rng);
            }
          } else if (p.dest === 'sigurdDestroyEnemy') {
            if (protectedFromOppLeave(st, a.k, p.chooser))
              return deny(`${nameOf(st, a.k)} ไม่ถูกนำออกจากสนามโดยความสามารถฝ่ายตรงข้าม`);
            const cCost = effCost(st, a.k) || 0;
            p.costGot = (p.costGot || 0) + cCost;
            p.multiGot = (p.multiGot || 0) + 1;
            destroyCard(st, fx, a.k, destroyOptsFromSrc(st, p.src, a.k));
            addLog(st, p.chooser, `เอฟเฟกต์ ${nameOf(st, p.src)}: ทำลาย ${nameOf(st, a.k)} (Cost ${cCost} · สะสม ${p.costGot}/${p.costSumMax})`);
            fx.snd = 'clash';
            if (promptCandidates(st, p).length) {
              st.prompts.unshift(p);
              addLog(st, p.chooser, `เลือกทำลายเพิ่มได้ หรือข้าม (Cost รวม ${p.costGot}/${p.costSumMax})`);
            }
      ...
      case 'skipPrompt': {
        ...
        if (p.dest === 'sigurdReturnHell') {
          if ((p.multiGot || 0) < 1) return deny('ต้องเลือก "เทพธิดาแห่งวัลฮัลลา" จากนรกอย่างน้อย 1 ใบ');
          finishSigurdField(st, fx, p, rng);
          break;
        }
      ...
      case 'reactYes': {
        ...
        if (p.reactTrigger === 'enemyDeclareAttack' || p.reactTrigger === 'oppBattlePhaseStart' || p.reactTrigger === 'ownAvatarFights' || p.reactTrigger === 'ownAvatarLeftField') {
          ...
          if (p.reactTrigger === 'ownAvatarFights' && st.inst[pick] && st.inst[pick].type === 'Avatar') {
            runOwnFightAvatarFromHand(st, fx, p, pick, rng);
            break;
          }
      ...
      case 'declareAttack': {
        ...
        if (pemuSpriteCombatLockApplies(st, oa, a.atk, a.def, isLife, isConstructAtk)) {
          declCtx._blockReact = true;
          skipDefAuto = true;
        }
        ...
        st.pending = {
          ...
          atk: a.atk, def: a.def || null, life: a.life || null, by: oa, target: ot, held: false,
          blockReact: !!declCtx._blockReact,
          blockDestroyPreventAfter: !!(declCtx._blockReact && pemuSpriteCombatLockApplies(st, oa, a.atk, a.def, isLife, isConstructAtk))
        };
        ...
        if (a.def && !isLife && !isConstructAtk) offerOwnFightReact(st, fx, ot, a.def);
        ...
        if (defCanRespond) {
          ...
          armDestroyPreventFromPending(st, pnd);
          resolveCombat(st, fx, pnd.atk, pnd.def, pnd.life);
          clearDestroyPreventLock(st);
        }
        break;
      }
      ...
      case 'activateAbility': {
        ...
        if (z.endsWith('.hand')) {
          ...
          if (costsH && costsH[0] && costsH[0].op === 'sendMagicToHell') {
            const need = costsH[0].count || 1;
            const filt = Object.assign({}, costsH[0].filter || {}, { _srcK: a.k, excludeSelf: true });
            const pChk = { kind: 'pick', from: 'ownMagic', src: a.k, chooser: ownerH, filter: filt, dest: 'magicToHellCost', excludeIds: [a.k] };
            if (promptCandidates(st, pChk).length < need)
              return deny(`บน Magic Zone ไม่ครบ ${need} ใบให้ส่งนรก`);
          }
          addLog(st, ownerH, `⚡ สั่งใช้จากมือ ${c.name}`);
          markOnce();
          if (costsH && costsH.length)
            payCostAndRunActivated(st, fx, ownerH, a.k, costsH, abH.actions || [], rng);
          else runHand();
          fx.snd = 'place'; break;
        }
        ...
        // จากมิติมืด (ไวรัส Blaster / Trojan / Creeper ฯลฯ)
        {
          const abDarkTrig = abilitiesOf(c.code, 'activatedFromDarkDimension', c.name)[0];
          if (z.endsWith('.dark') && abDarkTrig) {
            const ab = abDarkTrig;
            const ownerD = z[0];
            if (strict && isPlayer && ownerD !== by) return deny('สั่งใช้ได้เฉพาะการ์ดฝั่งตัวเอง');
            if (st.active !== ownerD) return deny('สั่งใช้ได้ในเทิร์นของคุณเท่านั้น');
            if (st.phase !== 'Main') return deny('สั่งใช้ได้เฉพาะ Main Phase');
            const cond = (ab.trigger && ab.trigger.if) || '';
            if (cond) {
              const mCond = cond.match(/^darkDimensionCountNameIncludes:(.+?)>=(\d+)$/);
              if (mCond) {
                const needle = mCond[1];
                const min = +mCond[2];
                const count = (st.zones[ownerD + '.dark'] || []).filter(id => {
                  const dc = st.inst[id]; return dc && nameMatches(dc, needle);
                }).length;
            ...
            const costsD = normalizeAbilityCost(ab.cost) || (Array.isArray(ab.cost) ? ab.cost : null);
            addLog(st, ownerD, `⚡ สั่งใช้จากมิติมืด ${c.name}`);
            if (costsD && costsD.length)
              payCostAndRunActivated(st, fx, ownerD, a.k, costsD, ab.actions || [], rng);
            else runActions(st, fx, ab.actions || [], { src: a.k, owner: ownerD, rng });
            fx.snd = 'place'; break;
          }
        }
        ...
        if (costList.length) {
          ...
          if (costOp.op === 'discard') {
            ...
            const hostK = c.attachedTo;
            if (!hostK || !st.inst[hostK]) return deny('ใช้ไม่ได้ — การ์ดไม่ได้สวมใส่ Avatar อยู่');
          } else if (costOp.op === 'sacrifice') {
      ...
      case 'endTurn': {
        ...
        for (const id in st.inst) {
          const c = st.inst[id];
          if (!c || !c.granted) continue;
          c.granted = c.granted.filter(g => g && g.until !== 'endOfTurn');
          if (!c.granted.length) delete c.granted;
        }
        ...
        if (st.blockLandPlayFor && st.blockLandPlayFor[ending]) delete st.blockLandPlayFor[ending];
        if (st.pemuSpriteCombatLock) delete st.pemuSpriteCombatLock[ending];
